...
// VGL works by cutting paths into tiles and evaulating trapezoidal coverage on the gpu.
// Edges use 8bit fixed-point coordinates (x0,y0,x1,y1), totalling 32bits per edge and are uploaded to a texture.
// Tiles are rendered with quads using gpu instancing, each tile contains an edge-list pointer, fill-info pointer
// along with the sign (inside/outside) state of it's bottom-left corner.
